...
from td3 import *
from mbpo_xyz import *
from mbpo_wpr import *
...
if __name__ == '__main__':
    ...
    MODEL = 'best_model_xyz.pt'
    ACTOR = 'best_actor_xyz.pt'
    CRITIC = 'best_critic_xyz.pt'
    ACTOR_ONNX = 'actor_xyz.onnx'

    # TD3 HYPERPARAMETERS
    INPUT_DIM = 6
    OUTPUT_DIM = 3
    ACTOR_HIDDEN_SIZE = [60, 60]
    CRITIC_HIDDEN_SIZE = [120, 120]
    LEARN_DELAY = 0
    POLICY_DELAY = 2
    TD3_GRAD_STEPS = 1
    TD3_BATCH_SIZE = 100
    ...
    OBS_LOW = [-1800, -1200, -100, -0.50, -0.50, -0.50]
    OBS_HIGH = [-1600, -400, 0, 0.50, 0.50, 0.50]
    ACTION_LOW = [-0.08, -0.08, -0.08]
    ACTION_HIGH = [ 0.08, 0.08, 0.08]
    ...
    test_args = [
        ...
        '500',
        '0',
        r'D:\Fanuc Experiments\mbpo-6d\test-0712-b\run-1\output\mbpo',
        r'D:\Fanuc Experiments\mbpo-6d\test-0712-b\run-1\output\mbpo',
        r'D:\Fanuc Experiments\mbpo-6d\test-0712-b\run-1\output\iteration_0',
        r'D:\LocalRepos\dotnet-fanuc-controller\PythonMBPO',
        r'D:\Fanuc Experiments\mbpo-6d\test-0712-b\run-1\output',
    ]
    ...
    args.data_dirs.append(r'D:\Fanuc Experiments\mbpo-6d\test-0712-b\warmup-1\output')
    ...
    for dir in args.data_dirs:
        for iter_dir in os.listdir(dir):
            ...
            if MBPO.iteration_prefix in iter_dir:
                ...
                csv_path = os.path.join(dir_, 'env_buffer_xyz.csv')
    ...
    env_path = os.path.join(args.model_dir, 'env_buffer_xyz.json')
    mbpo.env_buffer.save_buffer(env_path)
    env_path = os.path.join(args.model_dir, 'env_buffer_xyz.csv')
    ...
    if not args.warmup:
        ...
        buffer_path = os.path.join(args.iter_dir, 'combined_buffer_xyz.csv') 
    ...
    src = os.path.join(args.script_dir, 'iter_mbpo_xyz.py')
    dst = os.path.join(args.rl_dir, 'iter_mbpo_xyz.py')
    shutil.copy2(src, dst)
    src = os.path.join(args.script_dir, 'iter_mbpo_plot_xyz.py')
    dst = os.path.join(args.rl_dir, 'iter_mbpo_plot_xyz.py')
    shutil.copy2(src, dst)
...
#------------------------------------------------------------------ WPR----------------------------------
    ...
    ERROR = 'LineTrackError.csv'
    CONTROL = 'LineTrackMbpoControl.csv'
    MODEL = 'best_model_wpr.pt'
    ACTOR = 'best_actor_wpr.pt'
    CRITIC = 'best_critic_wpr.pt'
    ACTOR_ONNX = 'actor_wpr.onnx'
    ...
    # MBPO HYPERPARAMETERS
    OBS_LOW = [2.90, -0.15, 1.4, -0.002, -0.002, -0.002]
    OBS_HIGH = [3.20, 0.30, 1.60, 0.002, 0.002, 0.002]
    ACTION_LOW = [-0.003, -0.003, -0.003]
    ACTION_HIGH = [0.003, 0.003, 0.003]
    MODEL_HIDDEN_SIZE = [60, 50]
    MODEL_LR = 1e-3
    STOP_TRAINING_THRESHOLD = 2e-4
    MODEL_BUFFER_SIZE = 100_000
    ...
    #     '500',
    #     '0',
    #     r'D:\Fanuc Experiments\mbpo-6d\test-0711-b\run-1\output\mbpo',
    #     r'D:\Fanuc Experiments\mbpo-6d\test-0711-b\run-1\output\mbpo',
    #     r'D:\Fanuc Experiments\mbpo-6d\test-0711-b\run-1\output\iteration_0',
    #     r'D:\LocalRepos\dotnet-fanuc-controller\PythonMBPO',
    #     r'D:\Fanuc Experiments\mbpo-6d\test-0711-b\run-1\output',
    # ]
    ...
    for dir in args.data_dirs:
        for iter_dir in os.listdir(dir):
            ...
            if MBPO.iteration_prefix in iter_dir:
                dir_ = os.path.join(dir, iter_dir)
                mbpo.env_data_from_files([dir_], obs_space, act_space)
                csv_path = os.path.join(dir_, 'env_buffer_wpr.csv')
                mbpo.env_buffer.save_csv(csv_path)
                mbpo.env_buffer.reset(True)
                df = pd.read_csv(csv_path)
                rewards = df['reward'].tolist()
        ...
        #iter_dirs += [os.path.join(dir, iter_dir) for iter_dir in os.listdir(dir)]
    mbpo.env_data_from_files(iter_dirs, obs_space, act_space)
    env_path = os.path.join(args.model_dir, 'env_buffer_wpr.json')
    mbpo.env_buffer.save_buffer(env_path)
    env_path = os.path.join(args.model_dir, 'env_buffer_wpr.csv')
    mbpo.env_buffer.save_csv(env_path)
    ...
    if not args.warmup:
        ...
        combined_buffer = ReplayBuffer.combine([mbpo.env_buffer, mbpo.model_buffer])
        td3.train(combined_buffer, gradient_steps, verbose=True)  
        buffer_path = os.path.join(args.iter_dir, 'combined_buffer_wpr.csv') 
        combined_buffer.save_csv(buffer_path)
    ...
    # Copy python scripts to directory of iteration
    src = os.path.join(args.script_dir, 'iter_mbpo_wpr.py')
    dst = os.path.join(args.rl_dir, 'iter_mbpo_wpr.py')
    shutil.copy2(src, dst)
    src = os.path.join(args.script_dir, 'iter_mbpo_plot_wpr.py')
    dst = os.path.join(args.rl_dir, 'iter_mbpo_plot_wpr.py')
    shutil.copy2(src, dst)
