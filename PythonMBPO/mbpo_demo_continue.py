...
if __name__ == "__main__":
    ...
    eval_callback.init_callback(env, agent, mbpo)
    checkpoint_callback.init_callback(env, agent, mbpo)
    """File system"""
    # Directory names
    top_dir_name = f"220205_002_mbpo{hidden_size_model}_td3{hidden_size_actor}-{hidden_size_critic}_pendulumv0"
    top_dir = os.getcwd()
    rl_policy_save_dir = os.path.join(top_dir, "results", top_dir_name, "rl_policies")
    model_save_dir = os.path.join(top_dir, "results", top_dir_name, "models")
    data_save_dir = os.path.join(top_dir, "results", top_dir_name, "data")
    # Make directories
    ...
    agent.load_actor(prev_rl_actor_path)
    agent.load_critic(prev_rl_critic_path)
    mbpo.load_model(prev_model_path)
    ...
    for t in range(total_timesteps):
        ...
        # Evaluation
        eval_callback.best_actor_save_path = os.path.join(rl_policy_save_dir, f"best_actor_arch_{hidden_size_actor[0]}-{hidden_size_actor[1]}.pt")
        eval_callback.best_critic_save_path = os.path.join(rl_policy_save_dir, f"best_critic_arch_{hidden_size_critic[0]}-{hidden_size_critic[1]}.pt")
